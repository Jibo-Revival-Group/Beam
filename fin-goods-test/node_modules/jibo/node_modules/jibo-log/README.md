...
The callback here is called immediately and is only here for backwards
compatibility.
...
Logging to the cloud (DEPRECIATED)
----------------------------------
...
There is another call (`toCloudConnect`) which is required to complete
the connection to the cloud, and another call (`toCloudDisable`), but
I'm not going to go into them here because logging to the cloud this
way has been depreciated.

Use `log.toSyslog()` instead and let the syslog to cloud service take
care of getting it into the cloud.
...
be logged. It will log the details of the exception and then rethrow
the exception. It also registeres a listener for unhandled promise
errors.
...
are debugging such an issue you can temporarily add a call to:

```
log.disableUncaughtAndUnhandled();
```
