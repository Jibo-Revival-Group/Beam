...
if(typeof(query.opts) != 'undefined') {
  query = JSON.parse(unescape(query.opts));
}
...
var pubOptions = {
  ...
  headerNav : opts.headerNav || false,
  tutorialsOnly : query.tutorialsOnly || false,
  docsOnly : query.docsOnly || false,
  apiVersion : query.apiVersion || 'latest',
  indexUrl : conf.indexUrl || helper.getUniqueFilename('index')
};

var indexUrl = pubOptions.indexUrl;

var searchableDocuments = {};

var navigationMaster = {
  index: {
    title: pubOptions.systemName,
    link: indexUrl,
    members: []
  },
  tutorial: {
...
function addSignatureReturns(f) {
  if (pubOptions.methodHeadingReturns) {
...
function generate(docType, title, docs, filename, resolveLinks) {
  ...
  searchableDocuments[filename] = {
    ...
    "type" : docType,
    "path" :(pubOptions.tutorialsOnly ? '/docs/api/latest/' : '' ) + filename
  };
  
  if(!pubOptions.tutorialsOnly) {
...
function buildNav(members) {
  ...
  if (members.namespaces.length) {
    ...
    members.namespaces.forEach(function(n) {
      if (!hasOwnProp.call(seen, n.longname)) {
        ...
        nav.namespace.members.push(linkto(n.longname, n.longname));
      }
  ...
  if (members.interfaces && members.interfaces.length) {
    ...
    members.interfaces.forEach(function(m) {
      if (!hasOwnProp.call(seen, m.longname)) {
        ...
        nav.interface.members.push(linkto(m.longname, m.name));
      }
      seen[m.longname] = true;
    });
  ...
  if (members.tutorials.length) {
    nav.tutorial.sections = [];
    if(opts.hasOwnProperty('tutorials')) {
      var tutorialsJSON = fs.readFileSync(opts.tutorials + '/tutorials.json', 'utf8');
      var tutorialsObj = JSON.parse(tutorialsJSON);
      for( var sectionName in tutorialsObj){
        var section = { 'name' : sectionName, 'members' : {} };
        for(var t in tutorialsObj[sectionName]) {
          var tObj = tutorialsObj[sectionName][t];
          section.members[t] = { 'link' : tutoriallink(t) };
          if(tObj.hasOwnProperty('children')) {
            section.members[t].children = [];
            for(var c in tObj.children){
              section.members[t].children.push(tutoriallink(c));
            };
          }
        }
        nav.tutorial.sections.push(section);
      }
    }
  }
...
exports.publish = function(taffyData, opts, tutorials) {
  ...
  var sortOption = pubOptions.sort === undefined ? opts.sort : pubOptions.sort;
  sortOption = sortOption === undefined ? true : sortOption;
  sortOption = sortOption === true ? 'name, longname, version, since' : sortOption;
  if (sortOption) {
  ...
  if (pubOptions.disablePackagePath !== true && packageInfo && packageInfo.name) {
  ...
  if (sourceFilePaths.length) {
    var payload = pubOptions.sourceRootPath;
    if (!payload) {
  ...
  if (pubOptions.outputSourceFiles) {
  ...
  function generateTutorial(title, tutorial, filename) {
    ...
    searchableDocuments[filename] = {
      ...
      "body": searchData(html),
      "type" : "tutorial",
      "path" :(pubOptions.tutorialsOnly ? '/docs/' : '' ) + filename
    };
    // Don't publish index.html (the api index) if we are only building tutorials 
    if(!pubOptions.docsOnly && filename != 'index.html' || filename == 'index.html' && !pubOptions.tutorialsOnly) {
  ...
  fs.writeFileSync(outdir + '/scripts/searchdb.json', JSON.stringify(searchableDocuments), 'utf8');
