...
},{"./models/MusicLibrary":2,"./views/MusicView":3,"./views/StatusOverlay":4,"@be/be-framework":undefined,"jibo":undefined}],2:[function(require,module,exports){
...
class MusicView {
    ...
    setSlotFocus(slot, focused, animate) {
        ...
        if (focused) {
            ...
            css.opacity = '1';
            css.border = '3px solid #e8723a';
        }
        else {
            css.transform = 'scale(0.78)';
            css.webkitTransform = 'scale(0.78)';
            css.opacity = '0.45';
            css.border = '2px solid rgba(255,255,255,0.08)';
        }
    }
    bindAlbumSwipe(el) {
        const onStart = (x, y) => {
        ...
        const onMove = (x, y, e) => {
            ...
            if (Math.abs(dx) > Math.abs(dy) && Math.abs(dx) > 6) {
                ...
                this.dragOffset = dx;
                const damped = dx;
                this.setStripTranslate(this.restTranslate() + damped, false);
            }
        };
        const onEnd = (x) => {
            if (!this.touchActive) {
    ...
    selectTrack(i) {
        ...
        if (p && typeof p.catch === 'function') {
            p.catch((err) => {
                console.error('[jukebox] play() rejected:', err && err.name);
                this.statusEl.textContent = 'Error';
            });
        }
    }
