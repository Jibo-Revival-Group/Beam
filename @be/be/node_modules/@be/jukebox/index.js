...
class JukeboxSkill extends be_framework_1.BeSkill {
    ...
    finishOpen() {
        try {
            ...
            if (this.status) {
                this.status.setLoading(scan.albums.length
                    ? ('Found ' + scan.albums.length + ' album(s).\nOpening player...')
                    : 'No albums found yet.\nOpening player...');
            }
            const self = this;
            setTimeout(() => {
                try {
                    self.musicView = new MusicView_1.default(scan.albums, scan.detail);
                    if (self.status) {
                        self.status.dismiss();
                        self.status = null;
                    }
                    ...
                    console.error('[jukebox] MusicView failed:', err);
                    if (self.status) {
...
},{"./models/MusicLibrary":3,"./views/MusicView":4,"./views/StatusOverlay":5,"@be/be-framework":undefined,"jibo":undefined}],3:[function(require,module,exports){
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const jibo = require("jibo");
...
class MusicLibrary {
            ...
            for (let i = 0; i < entries.length; i++) {
                ...
                try {
                    ...
                    for (let k = 0; k < kids.length; k++) {
                        ...
                        const nested = MusicLibrary.scanAlbum(nestedId, name + ' — ' + kid, kidPath, assetPack, fs, path);
                        if (addAlbum(nested)) {
                            nestedFound++;
                        }
                    }
    ...
    static scanAlbum(id, titleSource, albumPath, assetPack, fs, path) {
...
},{"../audio/AudioSupport":1,"jibo":undefined}],4:[function(require,module,exports){
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const AudioSupport_1 = require("../audio/AudioSupport");
class MusicView {
    constructor(albums, scanDetail) {
        this.albumIndex = 0;
        this.tracks = [];
        this.trackIndex = -1;
        this.screen = 'albums';
        ...
        this.listStartY = 0;
        this.listStartX = 0;
        this.listStartTranslate = 0;
        this.listLastY = 0;
        this.listLastT = 0;
        this.listVelocity = 0;
        this.listMomentumRaf = 0;
        this.listViewportH = 406;
        this.objectUrl = null;
        this.playToken = 0;
        this.albums = albums || [];
    ...
    cleanup() {
        try {
            ...
            if (this.listMomentumRaf) {
                cancelAnimationFrame(this.listMomentumRaf);
            }
    ...
    buildAlbumsScreen() {
        ...
        this.stripEl.style.webkitTransform = 'translate3d(0,0,0)';
    ...
    createCoverSlot() {
        ...
        frame.style.webkitTransform = 'scale(0.78)';
    ...
    setStripTranslate(x, animate) {
        this.baseTranslate = x;
        const css = this.stripEl.style;
        if (animate) {
    ...
    animateShift(delta) {
        if (this.animating || !this.albums.length) {
            return;
        }
        this.animating = true;
    ...
    bindAlbumSwipe(el) {
        ...
        const onMove = (x, y, e) => {
            if (!this.touchActive || this.animating) {
                return;
            }
        ...
        el.addEventListener('mousedown', (e) => { onStart(e.clientX, e.clientY); }, false);
        ...
        el.addEventListener('mouseup', (e) => { onEnd(e.clientX); }, false);
        ...
        el.addEventListener('click', () => {
            if (this.touchMoved || this.animating) {
                return;
            }
    ...
    shiftAlbum(delta) {
        if (!this.albums.length || this.animating) {
            return;
        }
    ...
    updateAlbumCaption() {
        ...
        if (this.albumTitleEl) {
            this.albumTitleEl.textContent = album.title;
        }
        if (this.albumMetaEl) {
    ...
    buildTracksScreen() {
        ...
        this.trackHeaderCover.style.webkitUserDrag = 'none';
        ...
        this.trackListEl.style.touchAction = 'none';
        this.trackContentEl = document.createElement('div');
        this.applyStyle(this.trackContentEl, {
            position: 'absolute', left: '0', top: '0', width: '100%',
            willChange: 'transform'
        });
        this.trackContentEl.style.webkitTransform = 'translate3d(0,0,0)';
    ...
    bindTrackListScroll(viewport) {
        ...
        const onEnd = () => {
            ...
            if (Math.abs(this.listVelocity) > 0.05) {
                this.startListMomentum(this.listVelocity * 16);
            }
        };
    ...
    setListTranslate(y, animate) {
        this.listY = y;
        if (!this.trackContentEl) {
            return;
        }
        const css = this.trackContentEl.style;
        if (animate) {
    ...
    visibleTrackCount() {
        ...
        for (let i = 0; i < this.trackRowEls.length; i++) {
            if (this.trackRowEls[i] && this.trackRowEls[i].style.display !== 'none') {
                n++;
            }
        }
        return n;
    }
    startListMomentum(velocityPxPerFrame) {
        this.stopListMomentum();
    ...
    bindAudio() {
        ...
        this.audio.addEventListener('error', () => {
            ...
            console.error('[jukebox] audio error for', this.audio.src, 'code=', err && err.code, 'message=', err && err.message);
            if (this.statusEl) {
    ...
    resolvePlayable(track) {
        ...
        if (absPath) {
            try {
                const nodeRequire = AudioSupport_1.getNodeRequire();
                if (!nodeRequire) {
                    return Promise.reject(new Error('Node require is not available'));
                }
                const fs = nodeRequire('fs');
                if (fs.existsSync(absPath)) {
                    return Promise.resolve(AudioSupport_1.fileToBlobUrl(absPath, AudioSupport_1.mimeForFormat(format)));
                }
            }
            catch (err) {
    ...
    paintProgress(force) {
        ...
        if (force || curText !== this.lastTimeText) {
            ...
            if (this.currentTimeEl) {
                this.currentTimeEl.textContent = curText;
            }
        }
        ...
        if (force || totText !== this.lastTotalText) {
            ...
            if (this.totalTimeEl) {
                this.totalTimeEl.textContent = totText;
            }
        }
    }
    nextTrack() {
...
MusicView.COVER = 280;
MusicView.STRIDE = 340;
MusicView.STAGE_W = 1000;
MusicView.SNAP_MS = 280;
MusicView.ROW_H = 78;
exports.default = MusicView;

},{"../audio/AudioSupport":1}],5:[function(require,module,exports){
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
class StatusOverlay {
    static show(message) {
