...
module.exports = function (ts) {
	...
	Tsifier.prototype.checkSemantics = function (program) {
		...
		// NOTE: semantic (type-only) diagnostics are logged as warnings instead of
		// fatal errors. The jibo v14 runtime package ships no .d.ts files, so the
		// 2016-era skill produces many type-only diagnostics (e.g. extending an
		// untyped jibo base class). Emit still runs (noEmitOnError is off), so we
		// keep the transpiled output rather than failing the whole bundle.
		semanticDiagnostics.forEach(function (error) {
			console.warn('[tsify][type-warning] ' + new CompileError(error).message);
		});
