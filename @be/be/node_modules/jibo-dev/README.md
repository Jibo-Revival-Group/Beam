...
Tool designed to make sure a Flow files contains all it's references, like MIMs, FSTs, etc.
...
Tool designed to fix minor problems with .mim files. Right now, it only enforces a strict Prompt ID format.

```js
import {MimFixer} from 'jibo-dev';
const fixer = new MimFixer(process.cwd(), '/path/to/file.flow');
fixer.on('end', () => {
	//all done!
});
fixer.run('path/to/mim/folder');
```

* **constructor()** Constructor to create a fixer.
* **run(targetPath, verbose)** Begin fixing .mim files.

### `mim-fixer` Command-line

```bash
mim-fixer --target ./mims
```

**Arguments**

* `--verbose` or `-v` Run in verbose mode, see more logs.
* `--target` or `-t` The input directory of *.mim files, or path to a single .mim file.
